...
for index, r in chw_data.iterrows():
    chw_sql += (
        f"INSERT INTO public.chw VALUES('{r['chw_uuid']}', '{r['region']}');\n"
    )
...
for _ in range(number_of_patiens):
    patient = fake.hexify(text='p_^^^^^^^^')
    patient_uuids.append(patient)

# Create DataFrame for easier manipulation and population of SQL table
patient_data = pd.DataFrame(patient_uuids, columns=["patient_uuid"])
...
patient_data = pd.concat([patient_data, patient_data[-2:]])

for index, r in patient_data.iterrows():
    patient_sql += (
        f"INSERT INTO public.patient VALUES('{r['patient_uuid']}');\n"
    )
...
assessment_sql = """
CREATE TABLE IF NOT EXISTS public.patient_assessment(
...
# Note: By manipulating the divisor, the share of positive test results can be manipulated
def results():
    number = fake.pyint(min_value=1, max_value=100)
    mod = number % 6
    if mod == 0:
        outcome = True
    else:
...
for _ in range(number_of_records):
    assessment_id.append(fake.hexify(text='a_^^^^^^^^'))
...
def next_appointment(initial_assessment):
    ...
    for index, r in follow_ups.iterrows():
        follow_ups.at[index, "assessment_id"] = fake.hexify(text='a_^^^^^^^^')
...
follow_ups_treatment = pd.concat([follow_ups_treatment, follow_ups_treatment[-2:]])
...
for index, r in assessment_data.iterrows():
    assessment_sql += (
        f"INSERT INTO public.patient_assessment VALUES('{r['assessment_id']}','{r['date']}','{r['chw_id']}',"
