...
INSERT INTO dot.configured_entities VALUES('Muso','b05f1f9c-2176-46b0-8e8f-d6690f696b9b', 'ancview_danger_sign', 'anc', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.ancview_danger_sign','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','66f5d13a-8f74-4f97-836b-334d97932781', 'ancview_delivery', 'anc', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.ancview_delivery','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','638ed10b-3a2f-4f18-9ca1-ebf23563fdc0', 'ancview_pregnancy', 'anc', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select ap.*,
        ap.lmp as lmp_date,
...
INSERT INTO dot.configured_entities VALUES('Muso','8ccab0bf-383e-4e41-9437-2b1c5007ba80', 'ancview_pregnancy_visit', 'anc', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.ancview_pregnancy_visit','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','f41fe8ee-ee1c-49dd-ae3d-c473daf441d5', 'chv', 'core', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
with source_data as (
    select distinct
        reported_by,
...
INSERT INTO dot.configured_entities VALUES('Muso','6ba8075f-6f35-4ff1-be3a-4c75d0884bf4', 'fpview_follow_up', 'fp', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.fpview_follow_up','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','95bd0f60-ab59-48fc-a62e-f256f5f3e6de', 'fpview_registration', 'fp', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.fpview_registration','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','173793ff-491d-4c73-8d0b-3903a82d3796', 'hhview_visits', 'core', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select *
from {{ schema }}.hhview_visits','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','baf349c9-c919-40ff-a611-61ddc59c2d52', 'iccmview_assessment', 'iccm', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
-- select *
...
INSERT INTO dot.configured_entities VALUES('Muso','50f31569-f2fc-4dc6-af49-4268381e7c13', 'iccmview_assessment_follow_up', 'iccm', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
with source_data as (
...
select *
from source_data','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','d0645118-bd68-4eba-8ead-fad114be86b7', 'mnview_follow_up', 'mn', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select * from {{ schema }}.mnview_follow_up','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','57a9fd48-51d8-4dc0-bbd1-a6e0405696cd', 'mnview_registration', 'mn', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
select * from {{ schema }}.mnview_registration','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','fade2413-8504-443f-b161-1c5470fc1df3', 'patient', 'core', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
with source_data as (
    select
        uuid,
...
select *
from source_data','2021-12-07 00:00:00+00','2021-12-07 00:00:00+00','Matt');
INSERT INTO dot.configured_entities VALUES('Muso','eaea6e4c-a455-4f04-bb36-4bab0f6ba1a3', 'pncview_visits', 'pnc', '{{ config(materialized=''view'') }}
{% set schema = <schema> %}
with source_data as (
